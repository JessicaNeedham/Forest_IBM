...
# Martinez-Cano et al. 2016 - Michaelis-Menten function with max height cap
d2h_martcano <- function(dbh, p1 = 58.0, p2 = 0.73, p3 = 21.8, dbh_maxh){
  h <- (p1*min(dbh, dbh_maxh)^p2)/(p3 + min(dbh, dbh_maxh)^p2)
  return(h)
}

# Martinez-Cano et al. 2016 - Michaelis-Menten function without max height cap
...
###################################################################################
### These equations are only necessary in simulations with crown damage ###
